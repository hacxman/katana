...
display ref model bounds = do
  ...
  if qq > maxslices then error "uha"
      ...
      ortho (-bounds) bounds (-bounds) bounds (0.01) bounds
...
--  STL n facets <- loadSTL "32ce64c4-df76-11e2-9db0-5404a640b18f.stl"
  ...
  let model = map (\(Facet n x y z) -> (x, y, z)) facets
...
--                                       (realToFrac c) :: Vertex3 GLfloat
  ...
  displayCallback $= display la mlist model_size
  blend $= Enabled
